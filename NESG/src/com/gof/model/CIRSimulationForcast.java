...
public class CIRSimulationForcast extends IrModel {
	...
	private void setIrmodelAttributes(List<IrParamModelBiz> irParamModelBizList) {
		...
		try {
			this.rZero = irParamModelBizList.stream().filter(s -> s.getParamTypCd()==EHwParamTypCd.R_ZERO).map(s -> s.getParamVal()).mapToDouble(Double::doubleValue).toArray()[0];
			this.alpha = irParamModelBizList.stream().filter(s -> s.getParamTypCd()==EHwParamTypCd.ALPHA).map(s -> s.getParamVal()).mapToDouble(Double::doubleValue).toArray()[0];
			this.theta = irParamModelBizList.stream().filter(s -> s.getParamTypCd()==EHwParamTypCd.THETA).map(s -> s.getParamVal()).mapToDouble(Double::doubleValue).toArray()[0];
			this.sigma = irParamModelBizList.stream().filter(s -> s.getParamTypCd()==EHwParamTypCd.SIGMA).map(s -> s.getParamVal()).mapToDouble(Double::doubleValue).toArray()[0];
		}
		catch(Exception e) {
